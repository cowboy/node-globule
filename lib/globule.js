...
globule.match = function(patterns, filepaths, options) {
  ...
  return patterns.reduce(function(result, pattern) {
    var include = pattern.indexOf('!') !== 0;
    var filterfn = minimatch.filter(include ? pattern : pattern.slice(1), options);
    // The first character is not ! (inclusion). Add all matching filepaths
    // to the result set.
    if (include) {
      return _.union(result, filepaths.filter(filterfn));
    }
    // The first character is ! (exclusion). Remove any filepaths from the
    // result set that match this pattern, sans leading !.
    return result.filter(function(filepath) {
