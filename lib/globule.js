...
globule._rename = function(filepath, options) {
  ...
  if (options.ext) {
    filepath = filepath.replace(extDotRe[options.extDot], options.ext);
  }
  // Join filepath and destBase?
  if (options.destBase) {
    filepath = path.join(options.destBase, filepath);
  }
  return filepath;
};
