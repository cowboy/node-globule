...
globule.findMapping = function() {
  ...
  return globule.find(args, options, function(err, filepaths) {
    // Do that "Node.js" thing.
    // https://gist.github.com/cowboy/8417940
    if (err) {
      callback(err);
    } else {
      callback(null, globule.mapping(filepaths, options));
    }
  });
};
...
globule.findMappingSync = function() {
  ...
  var options = _.isPlainObject(args[args.length - 1]) ? args.pop() : {};
  // Generate mapping from found filepaths.
  return globule.mapping(globule.findSync(args, options), options);
};
