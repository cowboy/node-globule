...
// A path.join wrapper that always uses / slashes.
function join() {
...
Globule.prototype.pause = function() {
  ...
  this.paused = true;
  if (this.globInstance && !this.globInstance.found) {
    this.globInstance.pause();
  }
};
...
Globule.prototype._processNextPattern = function() {
  ...
  var match = function(filepath) {
    ...
    if (this.srcBase && this.prefixBase) {
      filepath = join(this.srcBase, filepath);
    }
  ...
  if (this.sync) {
    ...
    // On error, fail.
    this.globInstance.on('error', this._finish.bind(this));
  }
};
...
Globule.prototype._filterFilepath = function(filepath) {
  ...
  var relpath = this.srcBase && !this.prefixBase ? join(this.srcBase, filepath) : filepath;
  try {
...
function rename(dest, options) {
  ...
  if (options.destBase) {
    dest = join(options.destBase, dest);
  }
  return dest;
}
