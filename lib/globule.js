...
function processPatterns(patterns, options, fn) {
  ...
  each(patterns, function(pattern) {
    ...
    if (pattern.indexOf('!') !== 0) {
      result = union(result, fn(pattern));
      return;
    }
    ...
    result = lodashfilter(result, function(filepath) {
...
globule.match = function(patterns, filepaths, options) {
  ...
  patterns = isArray(patterns) ? flattenDeep(patterns) : [patterns];
  filepaths = isArray(filepaths) ? flattenDeep(filepaths) : [filepaths];
...
globule.find = function() {
  ...
  var options = isPlainObject(args[args.length - 1]) ? args.pop() : {};
  // If options.src was specified, use it. Otherwise, use all non-options
  // arguments. Flatten nested arrays.
  var patterns;
  if (options.src) {
    patterns = isArray(options.src) ? flattenDeep(options.src) : [options.src];
  } else {
    patterns = flattenDeep(args);
  }
