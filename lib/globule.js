...
function Globule() {
  ...
  patterns.forEach(function(pattern, index) {
    if (pattern.indexOf('!') === 0) {
      ...
      this.excludes[index] = minimatch.filter(pattern, options);
    } else {
      this.includes.push({
        index: index,
        pattern: pattern,
  ...
  async.eachSeries(
    this.includes,
    function(includeObj, next) {
      var result = [];
      this.emit('pattern', includeObj.pattern);
      this.globInstance = new glob.Glob(includeObj.pattern, this.globOptions);
      this.globInstance.on('match', function(filepath) {
        filepath = this._fixFilepath(filepath);
        if (all.indexOf(filepath) !== -1 || result.indexOf(filepath) !== -1) {
          this.emit('dupe', filepath);
          return;
        }
        this.emit('found', filepath);
        var exclude = !this._filterFilepath(filepath) || this.excludes.slice(includeObj.index + 1).some(function(filterfn) {
          var exclude = filterfn(filepath);
          if (exclude) { this.emit('exclude', filepath); }
          return exclude;
        }, this);
        if (!exclude) {
      ...
      this.globInstance.on('error', next);
      this.globInstance.on('end', function() {
        if (result.length === 0 && this.globInstance.nonull) {
          this.emit('match', includeObj.pattern);
          result.push(includeObj.pattern);
  ...
  // this.on('pattern', console.log.bind(console, '> pattern == %s =='));
  // this.on('found',   console.log.bind(console, '> match? %s'));
  // this.on('exclude', console.log.bind(console, '> exclude! %s'));
  // this.on('dupe',    console.log.bind(console, '> dupe! %s'));
...
Globule.prototype._fixFilepath = function(filepath) {
...
globule.find = function() {
  ...
  return new Globule(args, options, callback);
};
