...
function Globule() {
  ...
  this.srcBase = o.options.srcBase;
  this.cwd = o.options.cwd;
  this.filter = o.options.filter || null;
  this.sync = Boolean(o.options.sync);
  this.nonull = o.options.nonull;
  delete o.options.nonull;
...
Globule.prototype._match = function(filepath) {
  ...
  if (found.indexOf(filepath) !== -1) {
    // Keep track of duplicates.
    this._dupes.push(filepath);
  } else if (helpers.filterFilepath(filepath, excludes)) {
...
Globule.prototype._filter = function(filepath) {
  ...
  // If cwd was specified instead of srcBase, prefix the cwd temporarily,
  // for filtering.
  var relpath = this.cwd ? path.join(this.cwd, filepath) : filepath;
  try {
...
Globule.prototype._end = function() {
  ...
  if (!this.glob.nosort) {
    ...
    // Sort dupes too.
    if (this._dupes.length > 0) {
      this._dupes.sort(this.glob.nocase ? alphasorti : alphasort);
    }
  }
  // Emit per-pattern 'hit' and 'miss' events.
  var miss = this._found.length + this._dupes.length === 0;
  this.emit(miss ? 'miss' : 'hit', {
    matches: this._found.slice(0),
    dupes: this._dupes.slice(0),
    pattern: pattern,
  });
